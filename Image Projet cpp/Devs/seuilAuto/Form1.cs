...
namespace seuilAuto
{
    public partial class Form1 : Form
    {
        ...
        private void seuillageAuto_Click(object sender, EventArgs e)
        {
            ...
            ClImageNdg  
            bmp = convertBW(img, 25);
            imcomp = inverseImage(img);
            imseuil = seuillageOtsu(img);

            // Element structurant
            es = elemStructurantVoisinnage(8); //v8

		// Filtre
		out = dilatation(imcomp, es);
            es = elemStructurantVoisinnage(4); //v4
		out = fermeture(out, es);

            // Détection du nombre de trou
            imremp = remplissageTrous(out);
            imtrou = soustractionImgBinaire(imremp, out);
            euler = eulerNumber(imtrou);*/
        }
